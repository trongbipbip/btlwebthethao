...
exports.deleteCategory = async (req, res) => {
    try {
        if (!req.session.user || req.session.user.role !== 'admin') {
            return res.status(401).json({ success: false, message: 'Không có quyền thực hiện thao tác này' });
        }

        const categoryId = req.params.id;
        
        // Kiểm tra xem có tin tức nào thuộc danh mục này không
        const [newsCount] = await connection.promise().query(
            'SELECT COUNT(*) as count FROM news WHERE category_id = ?',
            [categoryId]
        );
        
        if (newsCount[0].count > 0) {
            req.session.error = 'Không thể xóa danh mục này vì có bài viết đang sử dụng. Vui lòng chuyển các bài viết sang danh mục khác trước khi xóa.';
        ...
        if (result.affectedRows > 0) {
            ...
            req.session.error = 'Không tìm thấy danh mục cần xóa';
            return res.redirect('/admin/categories');
        }
        
    } catch (error) {
        console.error('Error in deleteCategory:', error);
        req.session.error = 'Đã xảy ra lỗi khi xóa danh mục';
        return res.redirect('/admin/categories');
    }
};
...
// Xử lý toggle trạng thái tin tức
exports.toggleNewsStatus = async (req, res) => {
    try {
        const newsId = req.params.id;
        
        // Lấy thông tin tin tức hiện tại
        const [newsResult] = await connection.promise().query(
            'SELECT status FROM news WHERE id = ?',
            [newsId]
        );
        ...
        const currentStatus = newsResult[0].status;
        const newStatus = currentStatus === 'Đang hoạt động' ? 'Ngừng hoạt động' : 'Đang hoạt động';
        ...
        res.json({ success: true, message: 'Đã cập nhật trạng thái tin tức', newStatus: newStatus });

    } catch (error) {
        console.error('Error in toggleNewsStatus:', error);
        res.status(500).json({ success: false, message: 'Có lỗi khi cập nhật trạng thái tin tức' });
    }
};
